...
#include "macros.h"
  ...
  PUBLIC Geometric_SegmentTetrahedronIntersection !Elias
...
999 ERRORS("DiffusionAdvectionDiffusion_PostSolveOutputData",ERR,ERROR)
  ...
  SUBROUTINE Geometric_Intersection(controlLoop,err,error,*)
    ...
    INTEGER(INTG) :: numberOfVertices,nodeIdx,node,nic,adjacentElement
    TYPE(SOLVERS_TYPE), POINTER :: solvers
    ...
    TYPE(FIELD_VARIABLE_TYPE), POINTER :: fieldVariable
    TYPE(DOMAIN_TOPOLOGY_TYPE), POINTER :: topology
    TYPE(VERTICES_TYPE) :: vertices
    REAL(DP) :: P(3),Q(3),b(3),c(3),point(3)
    REAL(DP) :: T(3,3),A(3,3)
    REAL(DP) :: xLp(4),xLq(4),xLpoint(4)
    REAL(DP) :: tParam
    INTEGER :: adjacentElemIdx,counter,splineIdx
    ...
    DO equationsSetIdx=1,solverMappingDiffusion%NUMBER_OF_EQUATIONS_SETS
      ...
      DO nodeIdx=1,numberOfVertices
        node=topology%elements%elements(1)%element_nodes(nodeIdx)
    ...
    P=(/278.41,-119.14,792.14/)
    Q=(/277.919,-120.915,797.265/)
    ...
    b=P-vertices%vertices(4)%verticesCoordinates(:)
    CALL MatrixVectorProduct(A,b,c,err,error,*999)
    xLp(1:3)=c
    xLp(4)=1-xLp(1)-xLp(2)-xLp(3)

    b=Q-vertices%vertices(4)%verticesCoordinates(:)
    CALL MatrixVectorProduct(A,b,c,err,error,*999)
    xLq(1:3)=c
    xLq(4)=1-xLq(1)-xLq(2)-xLq(3)

    DO nic=1,4
      IF (xLq(nic)<0 .OR. xLq(nic)>1) THEN
        ...
        IF (tParam>0 .AND. tParam<1) THEN !TODO if the curve passes close enough to an edge of the element then we have 2 tParam that passes the conditions but they both give us almost the same point so no problem but the artery will pass two tissue elements from here.
          point(1)=P(1)+tParam*(Q(1)-P(1))
          point(2)=P(2)+tParam*(Q(2)-P(2))
          point(3)=P(3)+tParam*(Q(3)-P(3))
          adjacentElemIdx=nic
        END IF
      END IF
    END DO
    ! b=point-vertices%vertices(4)%verticesCoordinates(:)
    ! CALL MatrixVectorProduct(A,b,c,err,error,*999)
    ! xLpoint(1:3)=c
    ! xLpoint(4)=1-xLpoint(1)-xLpoint(2)-xLpoint(3)
    ! adjacentElemIdx=MINLOC(ABS(xLpoint), DIM=1,MASK=(xLpoint > 0))
    adjacentElement=topology%domain%decomposition%topology%elements%elements(1)%adjacent_elements(adjacentElemIdx)% &
      & adjacent_elements(1)

    P=point

    DO nodeIdx=1,numberOfVertices
      node=topology%elements%elements(adjacentElement)%element_nodes(nodeIdx)
      CALL Field_ParameterSetGetNode(geometricField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,1,1, &
        & node,1,x,err,error,*999)
      CALL Field_ParameterSetGetNode(geometricField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,1,1, &
        & node,2,y,err,error,*999)
    ...
    adjacentElement=topology%domain%decomposition%topology%elements%elements(adjacentElement)%adjacent_elements(adjacentElemIdx)% &
      & adjacent_elements(1)
  ...
  !Elias
  !>Calculates the intersection between a given tetrahedron and a segment.
  SUBROUTINE Geometric_SegmentTetrahedronIntersection(point,r1,r2,r3,r4,P,Q,err,error,*)

    !Argument variables
    ! TYPE(PROBLEM_TYPE), POINTER :: problem !<A pointer to the problem to set the problem specification for
    ! INTEGER(INTG), INTENT(IN) :: problemSpecification(:) !<The problem specification to set
    INTEGER(INTG) :: r1,r2,r3,r4,P,Q,point
    INTEGER(INTG), INTENT(OUT) :: err !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: error !<The error string
    !Local Variables
    ! TYPE(VARYING_STRING) :: localError
    ...
    !   CALL FlagError("Problem is not associated.",err,error,*999)
    ! END IF
    ! CALL INVERT(A,B,det,ERR,ERROR,*999)
...
999 ERRORS("DiffusionAdvectionDiffusion_ProblemSpecificationSet",err,error)
  ...
  END SUBROUTINE Geometric_SegmentTetrahedronIntersection
