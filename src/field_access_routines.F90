...
#include "macros.h"  
  ...
  INTEGER(INTG), PARAMETER :: FIELD_INTEGRATED_ROBIN_SET_TYPE=39 !<Stores integrated Robin values calculated from  R q_h which q_h comes from Robin condition, q=q_h-h*T \see FIELD_ROUTINES_ParameterSetTypes,FIELD_ROUTINES
  INTEGER(INTG), PARAMETER :: FIELD_ROBIN_CONDITIONS_CONVECTION_COEFF_SET_TYPE=40 !<The parameter set corresponding to the field Robin boundary conditions. h in q=q_h-h*T. q_h is stored in FIELD_BOUNDARY_CONDITIONS_SET_TYPE like Neumann BCs. \see FIELD_ROUTINES_ParameterSetTypes,FIELD_ROUTINES
  !>@}
  ...
  PUBLIC FIELD_NUMBER_OF_SET_TYPES,FIELD_VALUES_SET_TYPE,FIELD_BOUNDARY_CONDITIONS_SET_TYPE,FIELD_INITIAL_VALUES_SET_TYPE, &
    ...
    & FIELD_INTEGRATED_ROBIN_SET_TYPE,FIELD_ROBIN_CONDITIONS_CONVECTION_COEFF_SET_TYPE
