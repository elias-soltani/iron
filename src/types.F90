...
MODULE Types
  ...
  !Elias Contains the boundary nodes
  TYPE boundaryNodesType
    INTEGER(INTG) :: numberOfBoundaryNodes !<The total number of boundary nodes
    INTEGER(INTG), ALLOCATABLE :: nodes(:) !<Boundary node numbers
  END TYPE boundaryNodesType
  ...
  TYPE MeshComponentTopologyType
    ...
    TYPE(boundaryNodesType) :: boundaryNodes !<The nodes on the boundary. !Elias
...
!    INTEGER(INTG) :: ADJACENT_FACES(0:1) !<ADJACENT_FACES(0:1). The face number of adjacent faces. ADJACENT_FACES(0) is the face number adjacent in the -xi direction. ADJACENT_FACES(1) is the face number adjacent in the +xi direction. Old CMISS name NPL(2..3,0,nl).
  ...
  !Elias */
  TYPE VertexType
    REAL(DP) :: verticesCoordinates(3) !<The xyz coordinates of the tetrahedron vertices
  END TYPE VertexType

  TYPE VerticesType
    INTEGER(INTG) :: numberOFVertices !<Number of the vertices for the tetrahedron
    TYPE(VertexType), ALLOCATABLE :: vertices(:) !<Vertices of the tetrahedron
  END TYPE VerticesType

  TYPE IntersectionsType
    REAL(DP) ::  intersection(3) !<Intersection coordinates
  END TYPE

  TYPE CrossedElementsType
    INTEGER(INTG) :: elementNumber !<The tetrahedron element number that we want to find the intersections for.
    TYPE(IntersectionsType) :: intersections(2) !<The coordinates of the two intersections of a given curve and the tetrahedron.
  END TYPE CrossedElementsType

  TYPE CrossedType
    INTEGER(INTG) :: numberOfCrossedElements !Number of tetrahedron that the curve crosses.
    TYPE(CrossedElementsType), ALLOCATABLE :: crossedElements(:) !<The tetrahedron elements.
  END TYPE CrossedType
  !Elias /*
...
!    INTEGER(INTG) :: robinMatrixSparsity !<The sparsity type of the Robin integration matrices. \see SOLVER_ROUTINES_SparsityTypes,SOLVER_ROUTINES
  ...
  !\*Elias.  temporarily I want to store the 1D artery elements interacting with the surrounding 3D elements around them. Which artery element interacts with which tissue element. See coupling done in DIFFUSION_ADVECTION_DIFFUSION_POST_SOLVE.
  TYPE arteryElementType
    INTEGER(INTG) :: arteryElementNumber
    INTEGER(INTG) :: numberOfTissueElements
    INTEGER(INTG), ALLOCATABLE :: tissueElementNumbers(:)
    REAL(DP) :: arteryElementRadius
  END TYPE arteryElementType

  TYPE coupledElementsType
    INTEGER(INTG) :: numberOfArteryElements
    TYPE(arteryElementType), ALLOCATABLE :: arteryElementNumbers(:)
  END TYPE coupledElementsType
  !Elias *\
